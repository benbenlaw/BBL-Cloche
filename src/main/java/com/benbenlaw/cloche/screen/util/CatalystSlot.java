...
public class CatalystSlot extends ResourceHandlerSlot {
    ...
    public CatalystSlot(ResourceHandler<ItemResource> handler, IndexModifier<ItemResource> slotModifier, int handlerSlot, int xPosition, int yPosition) {
    ...
    public boolean mayPlace(ItemStack itemStack) {
        ...
        List<Optional<Ingredient>> seeds = ClocheRecipeCache.getRecipes().stream().map(ClocheRecipe::catalyst).filter(seed -> seed.isPresent() && seed.get().test(itemStack)).toList();
