...
public class SeedSlot extends ResourceHandlerSlot {
    ...
    public SeedSlot(Level level, ResourceHandler<ItemResource> handler, IndexModifier<ItemResource> slotModifier, int handlerSlot, int xPosition, int yPosition) {
    ...
    public boolean mayPlace(ItemStack itemStack) {
        ...
        List<Ingredient> seeds = recipes.stream()
                .map(ClocheRecipe::seed)
                .filter(seed -> seed.test(itemStack))
                .toList();
